...
/**
 * TVWiki Provider v2.2
 * [v2.2 수정 사항]
 * - Fix: 도메인 팩트 체크 시 최상위 루트(/)가 아닌 구조가 보장된 카테고리(/popular) 경로를 찌르도록 변경 (False Negative 해결)
 * - Update: 리다이렉트 발생 시 정규식을 이용해 응답 URL에서 순수 도메인 부분만 안전하게 추출하도록 로직 보강
 */
class TVWiki : MainAPI() {
    ...
    // v2.2: 동적 도메인 탐색 및 리다이렉트 갱신 로직 (경로 기반 검증으로 수정)
    private suspend fun checkAndUpdateDomain() {
        if (isDomainChecked) {
            println("[TVWiki v2.2] 도메인 체크 완료 상태. 현재 도메인 유지: $currentMainUrl")
            return
        }
        
        val testPath = "/popular" // v2.2: list_type 태그가 보장된 카테고리 경로
        println("[TVWiki v2.2] 도메인 유효성 검사 시작: $currentMainUrl$testPath")
        
        try {
            val res = app.get("$currentMainUrl$testPath", headers = commonHeaders)
            // 응답된 전체 URL에서 정규식을 통해 순수 도메인 부분(예: https://tvwiki7.net)만 안전하게 추출
            val finalUrl = Regex("^https?://[^/]+").find(res.url)?.value ?: currentMainUrl
            
            // v2.2: 실제 데이터 구조(list_type)가 있는지 팩트 체크
            if (res.isSuccessful && res.text.contains("list_type")) {
                if (currentMainUrl != finalUrl) {
                    println("[TVWiki v2.2] HTTP 리다이렉트 감지. 도메인 강제 갱신: $currentMainUrl -> $finalUrl")
                    currentMainUrl = finalUrl
                    mainUrl = currentMainUrl
                } else {
                    println("[TVWiki v2.2] 현재 도메인이 유효하고 실제 데이터가 존재합니다: $currentMainUrl")
                }
                isDomainChecked = true
                return
            } else {
                println("[TVWiki v2.2] 접속은 성공했으나 데이터가 없습니다(JS 리다이렉트/차단 페이지 의심). 새 도메인 탐색을 시작합니다.")
            }
        } catch (e: Exception) {
            println("[TVWiki v2.2] 현재 도메인 접속 실패, 새 도메인 탐색을 시작합니다. 에러: ${e.message}")
        }
        ...
        for (i in startNum..startNum + 20) {
            ...
            try {
                ...
                println("[TVWiki v2.2] 스캔 실패: $testUrl (${e.message})")
            }
        }
        
        println("[TVWiki v2.2] 도메인 탐색을 완료했지만 찾을 수 없습니다. 기존 도메인을 유지합니다.")
        isDomainChecked = true 
    }
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        return try {
            val doc = requestMutex.withLock {
                ...
                println("[TVWiki v2.2] 카테고리 로드 대기: ${request.name} | ${randomDelay}ms 지연 후 요청: $url")
                delay(randomDelay)
            ...
            if (list.isEmpty()) {
                println("[TVWiki v2.2] 메인 페이지 데이터 로드 결과가 0건입니다. 도메인 체크 플래그를 초기화합니다.")
                isDomainChecked = false
            } else {
                println("[TVWiki v2.2] 메인 페이지 로드 성공. 아이템 개수: ${list.size}")
            }
            
            newHomePageResponse(request.name, list, hasNext = list.isNotEmpty())
        } catch (e: Exception) {
            println("[TVWiki v2.2] Main page error: ${e.message}")
            isDomainChecked = false
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        println("[TVWiki v2.2] search: $searchUrl")
        return try {
            ...
            if (items.isEmpty()) {
                println("[TVWiki v2.2] 검색 결과가 없거나 데이터 파싱에 실패했습니다. (검색어: $query)")
            }
            items
        } catch (e: Exception) { 
            println("[TVWiki v2.2] Search 에러: ${e.message}")
            emptyList() 
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        if (!targetUrl.startsWith(mainUrl) && targetUrl.contains("tvwiki")) {
            ...
            println("[TVWiki v2.2] 과거 도메인 주소 감지. 최신 도메인으로 치환: $targetUrl")
        }

        println("[TVWiki v2.2] load 시작 - URL: $targetUrl")
        ...
        try {
            ...
            if (match != null) {
                ...
                println("[TVWiki v2.2] 터널링 포스터 복원 완료: $passedPoster")
            }
        ...
        println("[TVWiki v2.2] 포스터 유효성 검사 - 추출된포스터: $poster / 무효판정여부: $isInvalidPoster")

        if (isInvalidPoster && passedPoster != null) {
            println("[TVWiki v2.2] 상세페이지 포스터 무효 확인. 터널링된 포스터 우선 적용 진행.")
            poster = passedPoster
        }
        ...
        println("[TVWiki v2.2] 최종 적용된 줄거리(Plot) 텍스트 길이: ${story.length}")
    ...
    override suspend fun loadLinks(
        ...
        if (!targetData.startsWith(mainUrl) && targetData.contains("tvwiki")) {
            ...
            println("[TVWiki v2.2] loadLinks 과거 도메인 치환: $targetData")
        }
