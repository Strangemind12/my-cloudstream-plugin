...
class StreamedProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        println("[Streamed v1.7] 디버깅 - getMainPage 호출됨: url=${request.data}, page=$page")
        
        val document = app.get(request.data).document
        val homeList = arrayListOf<SearchResponse>()
        
        println("[Streamed v1.7] 디버깅 - 메인 페이지 HTML 파싱 시작")
    ...
    override suspend fun load(url: String): LoadResponse {
                ...
                if (absUrl.startsWith(baseUrl) && absUrl != baseUrl) {
                    if (absUrl.matches(Regex(""".*/(?:admin|delta|echo)/\d+/?$"""))) {
                        if (!sourceLinks.contains(absUrl)) {
                            ...
                            println("[Streamed v1.7] 디버깅 - a 태그에서 재생 링크 추출됨: $absUrl")
                        }
                    }
                }
            }
        ...
        // CSR 특성 상 링크가 DOM에 없을 확률이 높으므로, 요청하신 기본 구조를 강제 주입
        if (sourceLinks.isEmpty()) {
            println("[Streamed v1.7] 디버깅 - 정적 a 태그에서 파생 링크를 찾지 못해 지정된 기본 예측 URL(admin/1, admin/2, delta/1, echo/1)을 강제 생성합니다.")
        ...
        println("[Streamed v1.7] 디버깅 - load 파싱 완료. 앱 메인에 재생 버튼을 활성화시키기 위해 LiveStreamLoadResponse를 반환합니다.")
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        println("[Streamed v1.7] 디버깅 - loadLinks 호출됨 (재생 버튼 클릭됨): data=$data")
        
        val links = AppUtils.tryParseJson<List<String>>(data) ?: listOf(data)
        var isSuccess = false
        
        for ((index, sourceUrl) in links.withIndex()) {
            println("[Streamed v1.7] 디버깅 - 채널 우회 탐색 시작: $sourceUrl")
            ...
            if (!m3u8Url.isNullOrEmpty()) {
                ...
                println("[Streamed v1.7] 디버깅 - 최종 m3u8 매칭 및 캡처 성공: name=$sourceName, url=$m3u8Url")
                ...
                println("[Streamed v1.7] 디버깅 - 채널 $sourceUrl 에서 m3u8 캡처 실패 또는 404 발생.")
            }
        }
        
        if (!isSuccess) {
            println("[Streamed v1.7] 디버깅 - 모든 서버 탐색 완료. 추출된 링크가 없습니다.")
        }
    ...
    private suspend fun captureM3u8WithWebView(url: String, referer: String): String? = suspendCancellableCoroutine { cont ->
        ...
        handler.post {
            try {
                // TVWiki 브랜치 방식과 동일하게 Application Context 획득
                ...
                val timeoutRunnable = Runnable {
                    if (cont.isActive) {
                        println("[Streamed v1.7] 디버깅 - WebView 타임아웃 5초 도달 (강제 종료): $url")
                ...
                webView.webViewClient = object : WebViewClient() {
                    // 크롬 개발자 도구의 '네트워크' 탭처럼 모든 XHR, Fetch 리소스 요청을 가로챔
                    override fun shouldInterceptRequest(view: WebView?, request: WebResourceRequest?): WebResourceResponse? {
                        val reqUrl = request?.url?.toString() ?: ""
                        
                        if (reqUrl.contains(".m3u8")) {
                            println("[Streamed v1.7] 디버깅 - 백그라운드 m3u8 가로채기 완벽 성공: $reqUrl")
                    ...
                    // 존재하지 않는 Admin 번호에 대해 빠르게 포기하기 위함 (404 즉시 차단)
                    override fun onReceivedHttpError(view: WebView?, request: WebResourceRequest?, errorResponse: WebResourceResponse?) {
                        super.onReceivedHttpError(view, request, errorResponse)
                        if (request?.isForMainFrame == true && errorResponse?.statusCode == 404) {
                            println("[Streamed v1.7] 디버깅 - 프레임 404 에러 감지. 타임아웃 대기 없이 조기 종료: ${request.url}")
                            handler.removeCallbacks(timeoutRunnable)
                            handler.post {
                                if (cont.isActive) {
                                    try { webView.destroy() } catch (e: Exception) {}
                    ...
                    override fun onPageFinished(view: WebView?, url: String?) {
                        ...
                        // 플레이어 자동 재생 스크립트 강제 주입 (m3u8 패킷 발동 유도)
                        val clickScript = """
                            setTimeout(function() {
                                ...
                                iframes.forEach(function(iframe) {
                                    try {
                                        var ibtns = iframe.contentWindow.document.querySelectorAll('.play-btn, .plyr__control--overlaid, [role="button"], video');
                                        ibtns.forEach(function(btn) { btn.click(); });
                                    } catch(e) {}
                                });
                            }, 500);
                        """.trimIndent()
                ...
                println("[Streamed v1.7] 디버깅 - WebView 보이지 않는 백그라운드 렌더링 시작: $url")
                webView.loadUrl(url, mapOf("Referer" to referer))
            } catch (e: Exception) {
                println("[Streamed v1.7] 디버깅 - WebView 초기화 예외: ${e.message}")
