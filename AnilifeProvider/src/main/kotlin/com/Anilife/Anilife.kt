...
class Anilife : MainAPI() {
    ...
    private val commonHeaders = mapOf("User-Agent" to pcUserAgent, "Referer" to "$mainUrl/")
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        if (request.name.contains("TOP 20") && page > 1) return newHomePageResponse(request.name, emptyList(), false)
        ...
        return try {
            ...
            if (e is CancellationException) throw e // [공통 개선] 코루틴 취소 예외 상위 전달
    ...
    private fun parseCommonList(doc: Document): List<SearchResponse> {
        return doc.select(".listupd > article.bs").mapNotNull { element ->
            try {
                ...
                var poster = element.selectFirst("img")?.let { it.attr("src").ifEmpty { it.attr("data-src") } } ?: ""
                poster = fixUrl(poster).ifEmpty { "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" }
                ...
                newAnimeSearchResponse(title, finalHref, TvType.Anime) {
                    this.posterUrl = poster; this.posterHeaders = commonHeaders
                }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        try {
            ...
            val episodes = doc.select(".eplister > ul > li > a").reversed().mapNotNull { element ->
                ...
                if (currentEpisodeNumber == 0) currentEpisodeNumber = if (parsedNum > 0) parsedNum else 1
                else if (parsedNum > currentEpisodeNumber) currentEpisodeNumber = parsedNum else currentEpisodeNumber++
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        if (cleanData.contains("ref=")) {
            try { detailReferer = String(Base64.decode(cleanData.substringAfter("ref=").substringBefore("&"), Base64.NO_WRAP))
                  cleanData = cleanData.substringBefore("?ref=").substringBefore("&ref=")
            } catch (e: Exception) { }
        }

        val videoId = Regex("""(?:id|provider)/([^/?]+)""").find(cleanData)?.groupValues?.get(1) ?: "unknown_id"
        try {
            val webResponse = app.get(cleanData, headers = mapOf("Referer" to detailReferer, "User-Agent" to pcUserAgent))
            val playerUrl = AnilifeProxyExtractor().extractPlayerUrl(webResponse.text, mainUrl) ?: return false
            return AnilifeProxyExtractor().extractWithProxy(playerUrl, "https://anilife.live/", videoId, callback)
        } catch (e: Exception) {
            if (e is CancellationException) throw e
        }
        return false
