...
class MovieKing : MainAPI() {
    ...
    private fun Element.toSearchResponse(categoryName: String? = null): SearchResponse? {
        ...
        return if (type == TvType.Movie) {
            newMovieSearchResponse(title, finalHref, TvType.Movie) { this.posterUrl = fixedPoster }
        } else {
            newTvSeriesSearchResponse(title, finalHref, TvType.TvSeries) { this.posterUrl = fixedPoster }
        }
    }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return try {
            ...
            doc.select(".video-card").mapNotNull { it.toSearchResponse() }
        } catch (e: Exception) {
            if (e is CancellationException) throw e
            emptyList()
        }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        try {
            ...
            val rawTitle = doc.selectFirst("h3.title")?.text()?.trim() ?: doc.selectFirst("h1.text-secondary")?.text()?.trim() ?: "Unknown"
            val title = rawTitle.replace(titleRegex, "").trim()
            var poster = passedPoster ?: doc.selectFirst(".single-video-left img")?.attr("src") ?: doc.selectFirst("meta[property='og:image']")?.attr("content")
            val infoContent = doc.selectFirst(".single-video-info-content")

            fun getInfoText(keyword: String): String? {
                return infoContent?.select("p:contains($keyword)")?.text()?.replace(keyword, "")?.replace(":", "")?.trim()
            }
            ...
            return if (isMovie) {
                newMovieLoadResponse(title, realUrl, TvType.Movie, realUrl) {
                    this.posterUrl = fixUrl(poster ?: ""); this.plot = intro; this.tags = tagsList; this.year = year
                }
            } else {
