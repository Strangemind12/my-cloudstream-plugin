...
class TVMon : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        println("[TVMon][v1.11] getMainPage 요청: $url")
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        println("[TVMon][v1.11] 검색 실행: $query")
        
        val encodedQuery = try {
            URLEncoder.encode(query, "UTF-8")
        } catch (e: Exception) {
            println("[TVMon][v1.11] URL 인코딩 실패, 원본 쿼리 사용")
            query
        }
        ...
        return try {
            // [v1.11 핵심 개편] 조건부 판별을 버리고 무조건 메인 페이지 사전 핑 강제
            println("[TVMon][v1.11] 봇 방어 우회를 위한 메인 페이지 사전 핑(Ping) 1회 실행")
            app.get("$mainUrl/", headers = commonHeaders)
            
            println("[TVMon][v1.11] 세션 확보 완료. 실제 검색 요청 시도: $searchUrl")
            ...
            println("[TVMon][v1.11] 검색 결과 파싱 완료. 개수: ${items.size}")
            items
        } catch (e: Exception) {
            if (e is CancellationException) throw e
            println("[TVMon][v1.11] 검색 중 에러 발생: ${e.message}")
            emptyList()
        }
    }

    override suspend fun load(url: String): LoadResponse {
        println("[TVMon][v1.11] 상세 페이지 load 진입: $url")
    ...
    override suspend fun loadLinks(
        ...
        println("[TVMon][v1.11] loadLinks 진입")
        val doc = try {
        ...
        try {
            println("[TVMon][v1.11] WebViewResolver를 통한 동적 iframe 추출 시도")
            ...
            if (wbIframe != null) {
                ...
                println("[TVMon][v1.11] WebViewResolver 추출 성공: $playerUrl")
                if (playerUrl.contains("player.bunny-frame.online")) {
                     val extracted = BunnyPoorCdn().extract(fixUrl(playerUrl).replace("&amp;", "&"), data, subtitleCallback, callback, null)
                     if(extracted) return true
                }
            ...
            if (e is CancellationException) throw e
            println("[TVMon][v1.11] WebViewResolver 추출 예외: ${e.message}")
        }

        val thumbnailHint = extractThumbnailHint(doc)
        if (thumbnailHint != null) {
            try {
                println("[TVMon][v1.11] thumbnailHint를 이용한 우회 재생 시도")
    ...
    private suspend fun extractFromApi(
        ...
        try {
            ...
            println("[TVMon][v1.11] 세션 API 요청 시도")
            ...
            if (json != null && json.success && !json.playerUrl.isNullOrEmpty()) {
                ...
                println("[TVMon][v1.11] API 세션 획득 성공. Player URL: $fullUrl")
