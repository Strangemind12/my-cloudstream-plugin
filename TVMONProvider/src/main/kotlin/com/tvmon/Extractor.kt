...
/**
 * Version: v23.10 (Full Debugging & State Isolation)
 * Modification:
...
class BunnyPoorCdn : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        println("[TVMON][v23.10] getUrl 호출됨. URL: $url")
    ...
    suspend fun extract(
        ...
        synchronized(this) {
            if (currentProxyServer != null) {
                println("[TVMON] [INIT] 기존 실행 중인 프록시 서버 종료 시도.")
                currentProxyServer?.stop()
                currentProxyServer = null
                println("[TVMON] [INIT] 기존 프록시 서버 종료 완료.")
            } else {
                println("[TVMON] [INIT] 기존 실행 중인 프록시 없음.")
            }
        }

        println("[TVMON] [STEP 1] extract() 로직 시작.")
        var cleanUrl = url.replace(Regex("[\\r\\n\\s]"), "").trim()
        val cleanReferer = referer?.replace(Regex("[\\r\\n\\s]"), "")?.trim() ?: "https://tvmon.site/"
        println("[TVMON] [INFO] CleanUrl: $cleanUrl, Referer: $cleanReferer")
        
        // 2. Iframe / Embed URL 파싱
        if (!cleanUrl.contains("v/f/") && !cleanUrl.contains("v/e/")) {
            println("[TVMON] [CHECK] 직접 비디오 링크가 아님. Referer 페이지($cleanReferer)에서 bunny-frame 링크 탐색 시도.")
            try {
                val refRes = app.get(cleanReferer)
                println("[TVMON] [HTTP] Referer 페이지 응답 코드: ${refRes.code}")
                ...
                if (iframeMatch != null) {
                    ...
                    println("[TVMON] [FOUND] Iframe 링크 발견: $cleanUrl")
                } else {
                    println("[TVMON] [WARN] Iframe 링크 발견 실패. 원본 URL 사용.")
                }
            } catch (e: Exception) { 
                println("[TVMON] [ERROR] Iframe 파싱 중 예외 발생: ${e.message}") 
                e.printStackTrace()
            }
        }
        ...
        println("[TVMON] [DATA] 새로운 키 세션 저장소(Set) 생성 완료.")

        // 3. WebView 실행 (c.html이 없는 경우)
        if (!cleanUrl.contains("/c.html")) {
            println("[TVMON] [STEP 2] WebView 훅 실행 필요. runWebViewHook 호출.")
            val webViewResult = runWebViewHook(cleanUrl, cleanReferer, currentSessionKeys)
            if (webViewResult != null) {
                capturedUrl = webViewResult
                println("[TVMON] [RESULT] WebView 훅 성공. 캡처된 URL: $capturedUrl")
            } else {
                println("[TVMON] [FAIL] WebView 훅 실패 또는 타임아웃. 기존 URL 유지.")
            }
        } else {
            println("[TVMON] [SKIP] 이미 c.html 링크임. WebView 훅 건너뜀.")
        }
        ...
        if (capturedUrl != null) {
            ...
            if (!cookie.isNullOrEmpty()) {
                ...
                println("[TVMON] [INFO] 쿠키 발견 및 헤더 적용.")
            }

            try {
                println("[TVMON] [STEP 3] M3U8 플레이리스트 분석 시작.")
                var requestUrl = capturedUrl!!.substringBefore("#")
                println("[TVMON] [HTTP] M3U8 요청: $requestUrl")
                
                var response = app.get(requestUrl, headers = headers)
                println("[TVMON] [HTTP] M3U8 응답 코드: ${response.code}")
                var content = response.text.trim()

                // Master Playlist 처리
                if (!content.startsWith("#EXTM3U")) {
                    println("[TVMON] [CHECK] 내용이 #EXTM3U로 시작하지 않음. 내부 M3U8 탐색.")
                    Regex("""(https?://[^"']+\.m3u8[^"']*)""").find(content)?.let {
                        requestUrl = it.groupValues[1]
                        println("[TVMON] [FOUND] 내부 M3U8 발견: $requestUrl")
                ...
                if (content.contains("#EXT-X-STREAM-INF")) {
                    println("[TVMON] [CHECK] 멀티 비트레이트 파일 감지. 마지막 스트림 선택.")
                    ...
                    if (subUrlLine != null) {
                        ...
                        println("[TVMON] [RESOLVE] 서브 플레이리스트 URL: $requestUrl")
                        content = app.get(requestUrl, headers = headers).text.trim()
                    }
                }

                val isKey7 = content.lines().any { it.startsWith("#EXT-X-KEY") && it.contains("/v/key7") }
                println("[TVMON] [CHECK] 암호화 여부(Key7): $isKey7")

                if (isKey7) {
                    println("[TVMON] [STEP 4] 암호화 감지됨. 로컬 프록시 서버 구성 시작.")
                    ...
                    println("[TVMON] [PROXY] 프록시 서버 시작됨. 포트: ${newProxy.port}")
                    ...
                    println("[TVMON] [DATA] IV 추출 완료: $ivHex")
                    ...
                    println("[TVMON] [LOOP] M3U8 라인 리라이팅 시작.")
                        ...
                        if (trimmed.startsWith("#")) {
                            if (trimmed.startsWith("#EXT-X-KEY") && trimmed.contains("/v/key7")) {
                                ...
                                if (match != null) {
                                    ...
                                    // println("[TVMON] [REWRITE] Key URI 변경 완료.") 
                    ...
                    println("[TVMON] [DONE] 암호화되지 않은 영상. 직접 링크 반환.")
                ...
                println("[TVMON] [ERROR] M3U8 분석/처리 중 치명적 오류: ${e.message}")
                e.printStackTrace()
            }
        }
        println("[TVMON] [EXIT] extract() 종료. 처리 실패.")
        return false
    }
    ...
    private suspend fun runWebViewHook(url: String, referer: String, sessionKeys: MutableSet<String>) = suspendCancellableCoroutine<String?> { cont ->
        println("[TVMON] [WEBVIEW] runWebViewHook 시작. URL: $url")
        ...
        handler.post {
            try {
                ...
                val discoveryTimeout = Runnable {
                    if (cont.isActive) {
                        println("[TVMON] [WEBVIEW] 타임아웃 발생 (15초). c.html 발견 못함.")
                    ...
                    override fun onConsoleMessage(consoleMessage: ConsoleMessage?): Boolean {
                        ...
                        if (msg.startsWith("CapturedKeyHex:")) {
                            ...
                            if (sessionKeys.add(key)) { 
                                println("[TVMON] [WEBVIEW] 키 캡처 성공! Key: $key (Total: ${sessionKeys.size})")
                            }
                        }
                        return true
                    }
                ...
                webView.webViewClient = object : WebViewClient() {
                    ...
                    override fun shouldInterceptRequest(view: WebView?, request: WebResourceRequest?): WebResourceResponse? {
                        ...
                        // println("[TVMON] [WEBVIEW] 리소스 요청: $reqUrl")
                        
                        if (reqUrl.contains("/c.html") && reqUrl.contains("token=")) {
                            println("[TVMON] [WEBVIEW] c.html 패턴 발견! URL: $reqUrl")
                            detectedCUrl = reqUrl
                            
                            handler.removeCallbacks(discoveryTimeout)
                            println("[TVMON] [WEBVIEW] 타임아웃 타이머 해제됨.")
                            ...
                            thread {
                                try {
                                    runBlocking {
                                        println("[TVMON] [CHECK] 발견된 c.html 내용을 검증합니다.")
                                        ...
                                        if (!checkRes.text.contains("/v/key7")) {
                                            println("[TVMON] [CHECK] 일반 영상(No Key7) 확인. WebView 즉시 종료.")
                                            handler.post {
                                                if (cont.isActive) {
                                                    try { webView.destroy() } catch (e: Exception) {}
                                            ...
                                            println("[TVMON] [CHECK] Key7 영상 확인. 키 캡처를 위해 7초 대기.")
                                            handler.postDelayed({
                                                if (cont.isActive) {
                                                    println("[TVMON] [WEBVIEW] 키 수집 대기 종료. URL 반환.")
                                                    try { webView.destroy() } catch (e: Exception) {}
                                                    cont.resume(detectedCUrl)
                                                }
                                            }, 7000) 
                                    ...
                                    println("[TVMON] [ERROR] c.html 검증 중 오류: ${e.message}")
                                }
                            }
                        }
                    ...
                    override fun onPageFinished(view: WebView?, url: String?) {
                        ...
                        // println("[TVMON] [WEBVIEW] 페이지 로드 완료.")
                        view?.evaluateJavascript(hookScript, null)
                    }
                }

                println("[TVMON] [WEBVIEW] loadUrl 호출: $url")
                webView.loadUrl(url, mapOf("Referer" to referer))

            } catch (e: Exception) {
                println("[TVMON] [WEBVIEW] WebView 초기화 실패: ${e.message}")
    ...
    class ProxyWebServer(private val sessionKeys: MutableSet<String>) {
        ...
        fun start() {
            try {
                ...
                isRunning = true
                println("[TVMON] [PROXY] 서버 소켓 오픈 성공. Port: $port")
                
                thread(isDaemon = true) {
                    println("[TVMON] [PROXY] 요청 수신 대기 스레드 시작.")
                    while (isRunning) { 
                        try { 
                            val client = serverSocket!!.accept()
                            handleClient(client) 
                        } catch (e: Exception) {
                            if (isRunning) println("[TVMON] [PROXY] accept 에러: ${e.message}")
                        } 
                    }
                    println("[TVMON] [PROXY] 요청 수신 스레드 종료.")
                }
            } catch (e: Exception) {
                println("[TVMON] [PROXY] 서버 시작 실패: ${e.message}")
            }
        }
        ...
        fun setTestSegment(url: String) { 
            if (testSegmentUrl == null) {
                testSegmentUrl = url 
                println("[TVMON] [PROXY] 검증용 세그먼트 URL 설정됨: $url")
            }
        }
        ...
        private fun handleClient(socket: Socket) {
            try {
                ...
                // println("[TVMON] [PROXY] 클라이언트 요청 수신: $path")

                when {
                    path.contains("playlist.m3u8") -> {
                        println("[TVMON] [PROXY] 플레이리스트 요청 처리.")
                        output.write("HTTP/1.1 200 OK\r\nContent-Type: application/vnd.apple.mpegurl\r\nAccess-Control-Allow-Origin: *\r\n\r\n".toByteArray())
                        output.write(currentPlaylist.toByteArray())
                    }
                    path.contains("/key") -> {
                        println("[TVMON] [PROXY] 키 요청 수신.")
                        if (verifiedKey == null) {
                            println("[TVMON] [PROXY] 검증된 키가 없음. verifyMultipleKeys() 실행.")
                            verifiedKey = verifyMultipleKeys()
                        }
                        
                        if (verifiedKey != null) {
                            println("[TVMON] [PROXY] 키 반환 성공 (Verified).")
                        } else {
                            println("[TVMON] [PROXY] 키 검증 실패 또는 키 없음. 빈 바이트 반환.")
                        }
                    ...
                    path.contains("/seg") -> {
                        ...
                        // println("[TVMON] [PROXY] 세그먼트 다운로드 요청: $targetUrl")
                            ...
                            if (responseCode == 200) {
                                ...
                                if (bytesRead > 0) {
                                    ...
                                    if (offset != -1) {
                                        ...
                                        // println("[TVMON] [PROXY] MPEG-TS Sync 맞춤. offset=$offset")
                                    } else {
                                        output.write(buffer, 0, bytesRead)
                                        // println("[TVMON] [PROXY] MPEG-TS Sync 못 찾음. 원본 전송.")
                                    }
                                    inputStream.copyTo(output)
                                }
                                inputStream.close()
                            } else {
                                println("[TVMON] [PROXY] 원본 서버 오류 코드: $responseCode")
                            }
                        } catch (e: Exception) {
                            println("[TVMON] [PROXY] 세그먼트 스트리밍 중 에러: ${e.message}")
                        }
                    }
                }
        ...
        private fun verifyMultipleKeys(): ByteArray? = runBlocking {
            ...
            println("[TVMON] [VERIFY] 키 검증 프로세스 시작. 대상 URL: $url")
            println("[TVMON] [VERIFY] 현재 수집된 키 개수: ${sessionKeys.size}")
            ...
            try {
                ...
                println("[TVMON] [VERIFY] 테스트 세그먼트 다운로드 완료 (${responseData.size} bytes).")
                ...
                synchronized(sessionKeys) {
                    for ((index, hexKey) in sessionKeys.withIndex()) {
                        try {
                            ...
                            // println("[TVMON] [VERIFY] #${index+1} 테스트 중: $hexKey")
                            ...
                            for (offset in 0..512) {
                                ...
                                if (decrypted.size >= 377 && decrypted[0] == 0x47.toByte() && decrypted[188] == 0x47.toByte() && decrypted[376] == 0x47.toByte()) {
                                    println("[TVMON] [SUCCESS] 올바른 키 찾음! Key: $hexKey (Offset: $offset)")
                                    return@synchronized keyBytes
                                }
                            }
                        } catch (e: Exception) {
                            println("[TVMON] [WARN] 키 테스트 중 예외: ${e.message}")
                        }
                    }
                    println("[TVMON] [FAIL] 모든 키 테스트 실패.")
                    null
                }
            } catch (e: Exception) { 
                println("[TVMON] [ERROR] 검증 데이터 다운로드 실패: ${e.message}")
                null 
            }
        }
