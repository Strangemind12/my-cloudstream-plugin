...
class BunnyPoorCdn : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        println("[TVMON][v1.6] getUrl 호출됨. URL: $url")
    ...
    suspend fun extract(
        ...
        synchronized(this) {
            if (currentProxyServer != null) {
                println("[TVMON][v1.6] 기존 실행 중인 프록시 서버 종료 시도.")
                currentProxyServer?.stop()
                currentProxyServer = null
            }
        }

        println("[TVMON][v1.6] extract() 로직 시작.")
        var cleanUrl = url.replace(Regex("[\\r\\n\\s]"), "").trim()
        val cleanReferer = referer?.replace(Regex("[\\r\\n\\s]"), "")?.trim() ?: "https://tvmon.site/"
        
        if (!cleanUrl.contains("v/f/") && !cleanUrl.contains("v/e/") && !cleanUrl.contains("v/d/")) {
            println("[TVMON][v1.6] Referer 페이지($cleanReferer)에서 iframe 링크 탐색 시도.")
            try {
        ...
        if (!cleanUrl.contains("/c.html")) {
            println("[TVMON][v1.6] WebView 훅 실행 필요. runWebViewHook 호출.")
            val webViewResult = runWebViewHook(cleanUrl, cleanReferer, currentSessionKeys)
            if (webViewResult != null) {
                capturedUrl = webViewResult
                println("[TVMON][v1.6] WebView 훅 성공. 캡처된 URL: $capturedUrl")
            }
        }
        ...
        if (capturedUrl != null) {
            ...
            try {
                println("[TVMON][v1.6] M3U8 플레이리스트 분석 시작.")
                ...
                if (isKey7) {
                    println("[TVMON][v1.6] 암호화 감지됨. 하이브리드 로컬 프록시 구성 시작.")
                    ...
                    println("[TVMON][v1.6] 하이브리드 M3U8 라인 리라이팅 시작.")
                    ...
                    println("[TVMON][v1.6] 하이브리드 URL 반환: $finalUrl")
                    ...
                    println("[TVMON][v1.6] 일반 영상 다이렉트 링크 반환.")
                ...
                println("[TVMON][v1.6] 분석 치명적 오류: ${e.message}")
            }
        }
        return false
    }
                    ...
                    override fun onConsoleMessage(consoleMessage: ConsoleMessage?): Boolean {
                        ...
                        if (msg.startsWith("CapturedKeyHex:")) {
                            ...
                            if (sessionKeys.add(key)) { 
                                println("[TVMON][v1.6] 키 캡처 성공! Key: $key")
                            }
                        }
                        return true
                    }
    ...
    class ProxyWebServer(private val sessionKeys: MutableSet<String>) {
        ...
        fun start() {
            try {
                ...
                isRunning = true
                println("[TVMON][v1.6] 초경량 하이브리드 프록시 서버 시작. Port: $port")
            ...
            } catch (e: Exception) { println("[TVMON][v1.6] 프록시 시작 실패: ${e.message}") }
        }
        ...
        private fun verifyMultipleKeys(): ByteArray? = runBlocking {
            ...
            println("[TVMON][v1.6] 키 1회 선행 검증 시작. 대상 URL: $url")
                        ...
                        try {
                            ...
                            for (offset in 0..512) {
                                ...
                                if (decrypted.size >= 377 && decrypted[0] == 0x47.toByte() && decrypted[188] == 0x47.toByte() && decrypted[376] == 0x47.toByte()) {
                                    println("[TVMON][v1.6] 정답 키 매칭 성공! Key: $hexKey")
                                    return@synchronized keyBytes
                                }
                            }
                        } catch (e: Exception) {}
                    }
                    println("[TVMON][v1.6] 키 매칭 실패")
                    null
                }
            } catch (e: Exception) { 
                null 
